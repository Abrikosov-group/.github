...
SHA каждого автоматического запуска входит в ключ конкурентности: старое событие
не может отменить ревью более нового снимка PR. Перед анализом и публикацией
workflow сверяет текущий Head, а устаревший запуск не публикует ревью и не
перезаписывает статус нового снимка. Для точного Base/Head и модели результат
публикуется только один раз.
